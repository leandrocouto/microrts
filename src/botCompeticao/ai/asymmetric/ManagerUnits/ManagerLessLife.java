 ...
 * @author rubens
 */
public class ManagerLessLife extends ManagerAbstraction {

    private int _cenX, _cenY;

    public ManagerLessLife(int _playerID, int numUnits) {
    ...
    public void controlUnitsForAB(GameState state, HashSet<Unit> unidades) {
        ...
        if (unidades.isEmpty()) {
            Unit u = state.getUnit(getIDUnitLessLife(state));
            unidades.add(u);
        }
    ...
    private long getIDUnitLessLife(GameState state) {
