 ...
 */
package botCompeticao.ai.asymmetric.ManagerUnits;
 ...
 * @author rubens
 */
public class ManagerFartherEnemy extends ManagerAbstraction {

    private int _cenX, _cenY;

    public ManagerFartherEnemy(int _playerID, int numUnits) {
    ...
    public void controlUnitsForAB(GameState state, HashSet<Unit> unidades) {
        ...
        if (unidades.isEmpty()) {
            Unit u = state.getUnit(getIDUnitMoreFarther(state));
            unidades.add(u);
        }
    ...
    private long getIDUnitMoreFarther(GameState state) {
